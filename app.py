...
st.set_page_config(page_title="Geldium AI Collections Agent", layout="wide")
st.title("📉🛡️ CreditRiskAI - Intelligent Loan Delinquency Management Agent")
st.markdown("**Level 6 Agent with Reasoning • Memory • Tools • Reflection**")

# ====================== LOAD DATA ======================
@st.cache_data
def load_data():
    return pd.read_excel("C:/Users/HP/Projects/Gen AI Powered Data Analytics/Final_Dataset_with_Agent_Level5_Tools.xlsx")
...
with tab3:
    ...
    if history["interactions"]:
        history_df = pd.DataFrame(history["interactions"])
        st.dataframe(history_df, use_container_width=True)
    else:
        st.info("No previous interactions found for this customer.")

with tab4:
    st.subheader("🔄 Side-by-Side Customer Comparison")
    ...
    if st.button("Compare"):
        ...
        with colA:
            ...
            st.write("Missed Payments:", c1['Missed_Payments'])
            st.write("Credit Utilization:", f"{c1['Credit_Utilization']:.2f}")
        with colB:
            st.write(f"**{cust2}**")
            st.metric("Risk Score", calculate_risk_score(c2))
            st.write("Missed Payments:", c2['Missed_Payments'])
            st.write("Credit Utilization:", f"{c2['Credit_Utilization']:.2f}")

st.caption("Built as part of Tata iQ GenAI Powered Data Analytics Job Simulation • Level 6 Advanced Agent")